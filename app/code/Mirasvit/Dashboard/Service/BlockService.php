...
class BlockService
{
    ...
    public function getApiResponse(BlockInterface $block, array $filters)
    {
        ...
        if ($renderer === BlockInterface::RENDERER_TABLE) {
            ...
            $rTable    = $block->getConfig()->getTable()->getBaseTable() ?: $dimensionColumn->getTable()->getName();
        ...
        } elseif ($renderer === BlockInterface::RENDERER_CHART) {
            ...
            $dateColumn = $this->getDateColumn($rTable, $filters);
        } elseif ($renderer === BlockInterface::RENDERER_HTML) {
            return null;
        } else {
        ...
        if ($renderer === BlockInterface::RENDERER_SINGLE && $block->getConfig()->getSingle()->getCompare() && $dateColumn) {
        ...
        if ($renderer === BlockInterface::RENDERER_CHART && $block->getConfig()->getChart()->getCompare() && $dateColumn) {
            $previous = $this->ds->getPreviousInterval(
                $this->ds->toInterval($from, $to),
    ...
    {
        if ($block->getConfig()->getRenderer() === BlockInterface::RENDERER_SINGLE) {
        ...
        if ($block->getConfig()->getRenderer() === BlockInterface::RENDERER_TABLE) {
            $dimensions = $block->getConfig()->getTable()->getDimensions();
            if (count($dimensions) > 0) {
        ...
        if ($block->getConfig()->getRenderer() === BlockInterface::RENDERER_CHART) {
            $dimension = $block->getConfig()->getChart()->getDimension();
