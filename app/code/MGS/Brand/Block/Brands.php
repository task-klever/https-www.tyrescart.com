...
class Brands extends \Magento\Framework\View\Element\Template
{
    ...
    public function _construct()
    {
        ...
        $brandCollection = $brand->getCollection()
            ...
            ->addFieldToFilter('brand_category', ['in' => ['Tyres', 'Battery']])
    ...
    public function getBrandCollection($partsCategory)
    {
        $brandCollection = $this->_brand->getCollection()
            ->addFieldToFilter('status', 1)
            ->addStoreFilter($this->_storeManager->getStore()->getId())
            ->addFieldToFilter('brand_category', $partsCategory)
            ->setOrder('sort_order', 'ASC');
