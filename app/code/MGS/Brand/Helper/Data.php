...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function __construct(
        ...
        \MGS\Brand\Model\Patternmanagement $pattern,
    ...
    {
        return $this->scopeConfig->getValue('brand/general_settings/route', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
    }

    public function getPatternsByBrand($brandName)
    {
        $patternCollection = $this->pattern->getCollection()
    ...
    public function getCategoryByName($categoryId)
    {
        $category = $this->_categoryModel->load($categoryId);
        if ($category && $category->getId()) {
            return $category->getTitle(); // or getTitle() if using a custom field
        }
     return null; 
    }

    public function getCurrentBrandName(): string
    {
        $brand = $this->registry->registry('current_brand');
        return $brand ? $brand->getName() : '';
    }

    // Method for "Brands" breadcrumb
    public function getBrandsCrumbLabel()
    {
        return __('Brands');
    }
