...
class AbstractData extends \Tamara\Checkout\Helper\Core
{
    ...
    public function checkPaymentOptionsAvailability($countryCode, $currencyCode, $orderValue, $phoneNumber, $isVip = true, $storeId = 0) {
        ...
        $debugLog->info("checkPaymentOptionsAvailability called: country=$countryCode, currency=$currencyCode, orderValue=$orderValue, phone=$phoneNumber, storeId=$storeId");
        ...
        || \Tamara\Checkout\Gateway\Validator\CountryValidator::CURRENCIES_COUNTRIES_ALLOWED[$currencyCode] != $countryCode
            ...
            $allowedCountry = \Tamara\Checkout\Gateway\Validator\CountryValidator::CURRENCIES_COUNTRIES_ALLOWED[$currencyCode] ?? 'NOT_SET';
            $debugLog->info("TAMARA API SKIP: country mismatch - currency=$currencyCode expects=$allowedCountry got=$countryCode");
            return $result;
        }
        $adapter = $this->tamaraAdapterFactory->create($storeId);
        if ($adapter->getDisableTamara()) {
            $debugLog->info("TAMARA API SKIP: adapter disabled for store $storeId");
            return $result;
        }
        try {
            ...
            $debugLog->info("TAMARA API response success: " . ($response->isSuccess() ? 'true' : 'false') . " status: " . $response->getStatusCode());
            $result = $this->parsePaymentOptionsAvailabilityResponse($response, $currencyCode, $storeId);
            $debugLog->info("TAMARA API parsed result: " . json_encode($result));
        } catch (RequestException $requestException) {
            $debugLog->info("TAMARA API RequestException: " . $requestException->getMessage());
            $adapter->setDisableTamara(true);
        } catch (\Exception $exception) {
            $debugLog->info("TAMARA API Exception: " . $exception->getMessage());
            $this->getLogger()->debug(["Tamara" => $exception->getMessage()]);
        }
