...
class Available
{
    ...
    public function afterGetAvailableMethods(
        ...
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/tamara_debug.log');
        $debugLog = new \Zend_Log();
        $debugLog->addWriter($writer);

        if (is_null($quote)) {
            $debugLog->info('TAMARA REMOVED: quote is null');
            return $availableMethods;
        }
        if (!$this->config->isEnableTamaraPayment($quote->getStoreId())) {
            $debugLog->info('TAMARA REMOVED: payment not enabled for store ' . $quote->getStoreId());
            return $this->removeTamaraMethod($availableMethods);
        }
        if ($this->tamaraHelper->isAdminArea()) {
            $debugLog->info('TAMARA REMOVED: admin area');
            return $this->removeTamaraMethod($availableMethods);
        }

        if (!$this->tamaraHelper->isAllowedCurrency($quote->getCurrency()->getQuoteCurrencyCode(), $quote->getStoreId())) {
            $debugLog->info('TAMARA REMOVED: currency not allowed - ' . $quote->getCurrency()->getQuoteCurrencyCode());
            return $this->removeTamaraMethod($availableMethods);
        }
        ...
        if (!is_array($quoteItems)) {
            $debugLog->info('TAMARA: quoteItems not array, returning availableMethods');
            return $availableMethods;
        }
        foreach ($quoteItems as $item) {
            ...
            if (in_array($item->getProductId(), $excludeProductIds)) {
                $debugLog->info('TAMARA REMOVED: excluded product ID ' . $item->getProductId());
                return $this->removeTamaraMethod($availableMethods);
            }
        }

         // Check tabby_payment attribute - hide Tamara if any product has it set to "no"
         foreach ($quoteItems as $item) {
            $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            $fullProduct = $objectManager->create(\Magento\Catalog\Model\Product::class)->load($item->getProductId());
            $attrValue = $fullProduct->getData('tabby_payment');
            if ($attrValue == 0 || $attrValue === null || $attrValue === '') {
                $debugLog->info('TAMARA REMOVED: tabby_payment attribute is 0/null/empty for product ' . $item->getProductId() . ' value: ' . var_export($attrValue, true));
                return $this->removeTamaraMethod($availableMethods);
            }
        }

        // Only show Tamara if total is between 500 and 10000
        // if ($quote->getGrandTotal()) {
        //     $grandTotal = $quote->getGrandTotal();
        //     if ($grandTotal < 500 || $grandTotal > 10000) {
        //         $debugLog->info('TAMARA REMOVED: grand total out of range - ' . $grandTotal);
        //         return $this->removeTamaraMethod($availableMethods);
        //     }
        // }
        ...
        if ($this->config->isBlockWebViewEnabled($quote->getStoreId())) {
            if (!$this->isWebView($userAgent) || $this->isRestful()) {
                $debugLog->info('TAMARA REMOVED: block webview enabled');
                return $this->removeTamaraMethod($availableMethods);
            }
        }
        ...
        if ($this->config->getIsUseWhitelist($quote->getStoreId())) {
            if ($quote->getCustomerIsGuest()) {
                $debugLog->info('TAMARA REMOVED: whitelist enabled and guest customer');
                return $this->removeTamaraMethod($availableMethods);
            }

            $email = $quote->getCustomer()->getEmail();
            if (!$email || !$this->emailWhiteListRepository->isEmailWhitelisted($email)) {
                $debugLog->info('TAMARA REMOVED: email not whitelisted - ' . $email);
                return $this->removeTamaraMethod($availableMethods);
            }
        }

        $supportedMethods = $this->tamaraHelper->getPaymentTypesForQuote($quote);
        $debugLog->info('TAMARA supportedMethods: ' . json_encode($supportedMethods));
        if (empty($supportedMethods)) {
            $debugLog->info('TAMARA REMOVED: no supported methods from API');
            return $this->removeTamaraMethod($availableMethods);
        }
        $debugLog->info('TAMARA PASSED all checks - showing payment');
